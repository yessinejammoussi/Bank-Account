...
import org.bankAccount.domain.repository.AccountRepository;
...
public class AccountServiceTest {
    ...
    void checkBalance(double amount, double expected) {
        Account account = new Account( amount,null);
        Assertions.assertEquals(expected, accountServiceImpl.checkBalance(account));
    }

    @Test
    void getTransactions() {
        Transaction transaction = new Transaction(50,1,1);
        List<Transaction> transactions = new ArrayList<>();
        transactions.add(transaction);

        Account account = new Account( 50,transactions);
        Assertions.assertEquals(transactions, accountServiceImpl.getTransactions(account));
    }
}
